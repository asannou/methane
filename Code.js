...
function callGenerativeAI(userPrompt, projectContent) {
  ...
  systemPrompt += "  - 生成するソースコードの内容は、指示に関係のない改行、インデント、空白文字などを勝手に変更したり除去したりせず、可能な限り元のフォーマットを維持してください。特にHTMLファイルでは、元の構造とインデントを厳密に保持してください。 生成されるコードは必ず構文的に有効で、完全なものとしてください。特に、ソースコード内のコメント、空行、ブロックのインデントなどは重要です。\n";
